function h = plotflowdir(fdir, res, X, Y, ind, gage, basin, rivs)
...
% - basin boundary
%
% The gauge location plotting is not really working right now (8/18/2017)
%
% INPUTS
% fdir, flow direction raster (plain text format)
    ...
    for j=1:size(X,2)
        ...
        if fdir(i,j)>=0 % condition for being in the basin boundary
            plot([X(i,j)-interval,X(i,j)+interval,...
                ...
                Y(i,j)+interval,Y(i,j)+interval,Y(i,j)-interval],'k-')            
            hold on
